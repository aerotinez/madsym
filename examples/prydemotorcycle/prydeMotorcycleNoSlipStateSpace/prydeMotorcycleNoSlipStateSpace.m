function sys = prydeMotorcycleNoSlipStateSpace(p)
   ...
   M = prydeMotorcycleNoSlipSSMassMatrix(p);
   H = prydeMotorcycleNoSlipSSForcingMatrix(p);
   G = prydeMotorcycleNoSlipSSInputMatrix(p);
   ...
   sys.Offsets.u = [
      (p(1,:)*p(2,:)*p(15,:)^2*p(55,:)*p(57,:) + p(1,:)*p(2,:)*p(15,:)^2*p(55,:)*p(59,:))/(2*p(24,:));
      0;
   ];
