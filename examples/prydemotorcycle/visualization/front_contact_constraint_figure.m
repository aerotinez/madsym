...
%% Plot frames
plotFrame(axe,rigidtform3d(I,pf),[0,0,0]);

Cf0 = pf - (rf - tf).*Rf(:,3) - tf.*I(:,3);
plotFrame(axe,rigidtform3d(Rf,pf),m.blue);
plotFrame(axe,rigidtform3d(Rf*rotx(rad2deg(camber_f)).',Cf0),m.blue,':');

R = Rf*roty(reach).';
Cf = pf - (rf - tf).*R(:,3) - tf.*I(:,3);
plotFrame(axe,rigidtform3d(R,pf),m.orange);
plotFrame(axe,rigidtform3d(R*rotx(rad2deg(camber_f)).',Cf),m.orange,':');
